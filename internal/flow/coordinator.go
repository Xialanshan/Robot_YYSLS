...
func (c *Coordinator) SelectStyles(groupID, userID, text string, now time.Time) (StyleSelection, error) {
	...
	if !ok {
		return StyleSelection{Reply: c.at(userID) + " 请先发送「@机器人 计算毕业率」开始计算。"}, nil
	}
	if sess.State != session.StateChoosingStyles {
		return StyleSelection{Reply: c.at(userID) + " 当前会话不能选择流派，请发送「重新开始」后再试。"}, nil
	}

	names := splitStyleNames(text)
	styles, err := c.Styles.MustResolveNames(names)
	if err != nil {
		return StyleSelection{Reply: c.at(userID) + " 流派名称有误：" + err.Error()}, nil
	}
	...
	builder.WriteString(c.at(userID))
...
func (c *Coordinator) at(userID string) string {
	return "@" + userID
}
